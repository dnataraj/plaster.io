...
#import "TSLBase64-iOS/NSData+TSBase64.h"
#import "TSLPlasterGlobals.h"

@implementation TSLPacketSerializer

+ (const char *)JSONWithTextPacket:(NSString *)packet {
    return [TSLPacketSerializer JSONWithTextPacket:packet sender:nil];
}
...
+ (const char *)JSONWithImagePacket:(UIImage *)packet sender:(NSString *)sender {
    ...
    NSData *pngRep = UIImagePNGRepresentation(packet);
    if (pngRep) {
        NSMutableDictionary *kvDictionary = [NSMutableDictionary dictionary];
        ...
        NSData *imageData = [[NSData alloc] initWithData:pngRep];
        ...
        if (packet64 && [packet64 isKindOfClass:[NSString class]]) {
            ...
            if (packet) {
                ...
                if ([type isEqualToString:TSPlasterTypeText] || [type isEqualToString:TSPlasterTypeNotification]) {
                    ...
                    UIImage *image = [[UIImage alloc] initWithData:packet];
