...
@implementation TSLRedisController {
...
- (id)initWithIPAddress:(NSString *)ipv4Address port:(NSUInteger)port {
    self = [super init];
    if (self) {
        self.redisHostAddress = ipv4Address;
        _redisPort = port;
...
- (redisContext *)connect {
    ...
    redisContext *context = redisConnect([self.redisHostAddress UTF8String], (uint)[self redisPort]);
...
- (redisAsyncContext *)connectAndDispatch {
    signal(SIGPIPE, SIG_IGN);
    const char *temp = [self.redisHostAddress UTF8String];
    char *addr = (char *)malloc(sizeof(char) * strlen(temp));
    if (addr == NULL) {
        DLog(@"REDIS: Error establishing connection : %@", self.redisHostAddress);
    }
    strcpy(addr, temp);
    redisAsyncContext *localAsyncCtx = redisAsyncConnect(addr, (uint)[self redisPort]);
    if (localAsyncCtx->err) {
        DLog(@"REDIS: Error establishing connection : %s", localAsyncCtx->errstr);
        free(addr);
        return NULL;
    }
